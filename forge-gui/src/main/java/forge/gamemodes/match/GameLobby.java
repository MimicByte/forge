...
public abstract class GameLobby implements IHasGameType {
    ...
    public List<GameStartError> validateDedicatedStart(final GameType deckFormatType) {
        ...
        int humans = 0;
        int archenemies = 0;
        final boolean autoGeneratedDecks = hasAutoGeneratedVariant();
        ...
        for (int i = 0; i < getNumberOfSlots(); i++) {
            ...
            humans++;
            if (hasVariant(GameType.Archenemy) && slot.isArchenemy()) { archenemies++; }
            if (slot.getType() != LobbySlotType.REMOTE && slot.getType() != LobbySlotType.AI) {
                errors.add(new GameStartError(i, "Only remote human and configured AI seats can start a dedicated match."));
            } else if (slot.getDeck() == null && !autoGeneratedDecks) {
                errors.add(new GameStartError(i, slot.getName() + ": choose a deck."));
            } else if (!autoGeneratedDecks && checkLegality) {
                ...
                if (hasVariant(GameType.ArchenemyRumble)) {
                    problem = DeckFormat.getSchemeSectionConformanceProblem(slot.getDeck().get(DeckSection.Schemes));
                    if (problem != null) { errors.add(new GameStartError(i, slot.getName() + ": " + problem)); }
                }
            }
